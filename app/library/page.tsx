...
export default function LibraryPage() {
  ...
  return (
    <div className="min-h-screen py-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
      ...
      <div className="relative p-6 sm:p-10 rounded-3xl bg-slate-900/80 border border-white/10 overflow-hidden shadow-2xl backdrop-blur-xl">
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-500/20 text-rose-300 text-xs font-bold uppercase tracking-wider mb-2">
                <Sparkles className="w-3.5 h-3.5" /> Crunchyroll Watchlist Dashboard
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
            <div className="p-4 rounded-2xl bg-slate-950/60 border border-white/5 space-y-1">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Check className="w-3.5 h-3.5 text-emerald-400" /> Movies Watched
              </span>
            ...
            <div className="p-4 rounded-2xl bg-slate-950/60 border border-white/5 space-y-1">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5 text-sky-400" /> Total Watch Time
              </span>
              <p className="text-2xl font-black text-white">{formatMinutesToHours(stats.totalMinutes)}</p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-950/60 border border-white/5 space-y-1">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Star className="w-3.5 h-3.5 text-amber-400" /> Mean Rating
              </span>
              <p className="text-2xl font-black text-amber-400">{stats.averageRating > 0 ? `${stats.averageRating}/10` : 'N/A'}</p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-950/60 border border-white/5 space-y-1">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <BookmarkCheck className="w-3.5 h-3.5 text-rose-400" /> Plan to Watch
              </span>
              <p className="text-2xl font-black text-white">{stats.planToWatchCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div className="flex items-center gap-2 bg-slate-900 p-1.5 rounded-2xl border border-white/10">
          {[
            { id: 'completed', label: 'Watched / Completed', icon: Check },
            { id: 'plan_to_watch', label: 'Plan to Watch', icon: BookmarkCheck },
            { id: 'watching', label: 'Currently Watching', icon: Eye },
      ...
      {filteredMovies.length > 0 ? (
        ...
        <div className="py-20 flex flex-col items-center justify-center text-center space-y-3">
          <BookmarkCheck className="w-12 h-12 text-slate-700" />
          <p className="text-lg font-bold text-slate-300">No movies found in this list</p>
